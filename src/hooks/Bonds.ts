...
import { IUserBondDetails } from "../app/AccountSlice";
import { Bond, BondType } from "../lib/Bond";
import { IBondDetails } from "../app/BondSlice";
...
function useBonds(networkId: NetworkId) {
  ...
  const filteredBonds = allBonds.filter((bond:any) => networkId in bond.networkAddrs);
  ...
  useEffect(() => {
    ...
    const mostProfitableBonds = bondDetails.concat().sort((a, b) => {
      ...
      if (a["name"] == 'usdb') return 1
      else if (b["name"] == 'usdb') return -1
